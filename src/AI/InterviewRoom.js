...
const InterviewRoom = () => {
  ...
  const [isSpeechWebSocketReady, setIsSpeechWebSocketReady] = useState(false); // Track WebSocket connection state
  ...
  const setupWebSockets = () => {
    ...
    speechWebSocketRef.current.onopen = () => {
      ...
      setIsSpeechWebSocketReady(true); // Mark WebSocket as ready
    };

    speechWebSocketRef.current.onerror = (error) => {
      console.error('Speech WebSocket error:', error);
      setIsSpeechWebSocketReady(false); // Mark WebSocket as not ready on error
    };

    speechWebSocketRef.current.onclose = () => {
      console.log('Speech WebSocket closed');
      setIsSpeechWebSocketReady(false); // Mark WebSocket as not ready on close
    };
  };
  ...
  const handleNextQuestion = async () => {
    ...
    if (nextIndex < questions.length) {
        ...
        return new Promise((resolve) => {
          const checkWebSocket = () => {
            if (speechWebSocketRef.current && speechWebSocketRef.current.readyState === WebSocket.OPEN) {
              ...
              setTimeout(checkWebSocket, 100); // Check again after 100ms
            }
          };
          checkWebSocket();
        });
      ...
      try {
        await waitForWebSocket(); // Wait for WebSocket to be ready
