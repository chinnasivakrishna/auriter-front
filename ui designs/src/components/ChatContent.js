...
const ChatContent = () => {
  ...
  const transcribeAudio = async (audioBlob) => {
    try {
      ...
      const response = await fetch('https://auriter-back.onrender.com/api/chat/transcribe', {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
  ...
  const speakText = async (text, messageId) => {
    try {
      ...
      const response = await fetch('https://auriter-back.onrender.com/api/chat/synthesize', {
        method: 'POST',
        headers: {
